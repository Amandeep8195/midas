	...
	while ( have_posts() ) :
		...
		?>
				...
				if ( ! empty( $post->post_parent ) ) :
					...
					$post_title = sprintf( __( 'Go to %s', 'midastouch' ), strip_tags( get_the_title( $post->post_parent ) ) );
					?>
					<p class="page-title"><a href="<?php echo esc_url( get_permalink( $post->post_parent ) ); ?>" title="<?php echo esc_attr( $post_title ); ?>" rel="gallery">
						...
						printf( __( '<span class="meta-nav">&larr;</span> %s', 'midastouch' ), get_the_title( $post->post_parent ) );
						?>
					</a></p>
				<?php endif; ?>
					...
					<div class="entry-meta">
						<?php
							printf(
								...
								__( '<span class="%1$s">By</span> %2$s', 'midastouch' ),
						...
						?>
							...
							printf(
								...
								__( '<span class="%1$s">Published</span> %2$s', 'midastouch' ),
							...
							if ( wp_attachment_is_image() ) {
								...
								printf(
									...
									__( 'Full size is %s pixels', 'midastouch' ),
									sprintf(
										'<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
										esc_url( wp_get_attachment_url() ),
										esc_attr( __( 'Link to full-size image', 'midastouch' ) ),
										$metadata['width'],
										$metadata['height']
									)
								);
		...
		if ( wp_attachment_is_image() ) :
			...
			?>
						<p class="attachment"><a href="<?php echo esc_url( $next_attachment_url ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment">
							<?php
							/**
							 * Filters the Midas Touch default attachment width.
							 *
							 * @since Midas Touch 1.0
							 *
							 * @param int The default attachment width in pixels. Default 900.
							 */
							$attachment_width = apply_filters( 'midastouch_attachment_size', 900 );
							/**
							 * Filters the Midas Touch default attachment height.
							 *
							 * @since Midas Touch 1.0
							 *
							 * @param int The default attachment height in pixels. Default 900.
							 */
							$attachment_height = apply_filters( 'midastouch_attachment_height', 900 );
